...
def start_api(**kwargs):
    ...
    db = sqlite3.connect(resources + "riots-memory.db", check_same_thread=False)
    c = db.cursor()
    ...
    checkpointPath = resources + "logs/voices/"
    highest = 0
    ...
    tts_model = TTS(language='EN', device='auto', config_path=resources + "logs/voices/config.json", ckpt_path=resources + f"logs/voices/G_{highest}.pth")
...
async def push_messages():
    ...
    with(open(fresources + "{speaker}messages.json", "w")) as f:
...
def run_social_media(action: str = "atComment", test_mode: bool = True):
    ...
    if action == "original":
        ...
        # check user feed for mentions

    elif action == "atComment":
        print("Running At Comment", file=sys.stderr)
        atProtoClient = atproto.Client()
        atProtoClient.login(login=config["BLUESKY_HANDLE"], password=config["BLUESKY_PASSWORD"])
        timeline = atProtoClient.get_timeline(limit=30)
        for post in timeline.feed:
            seen = c.execute("SELECT * FROM atprotoSeen WHERE id = ?", (post.post.cid,)).fetchone()
            
            if seen:
                print(f"Skipping seen post {post.post.cid}", file=sys.stderr)
                continue

            if post.post.embed and post.post.embed.py_type == "app.bsky.embed.video#view":
                print(f"Skipping video post {post.post.embed.py_type}", file=sys.stderr)
                continue

            if post.reply:
                print(f"Skipping reply post by {post.reply.root.author.handle}", file=sys.stderr)
                continue

            print(f"\nPost: {post.post.record.text}", file=sys.stderr)
            print(f"Author: {post.post.author.handle}", file=sys.stderr)
            images = []
            if post.post.embed and "images" in post.post.embed:
                for image in post.post.embed.images:
                    response = requests.get(image.fullsize, stream=True)
                    images.append(response.raw.read())
                    print(f"Image: {image.fullsize}", file=sys.stderr)

            author = check_person(post.post.author.did, platform="bluesky")
            print(f"Author: {author.username} - {post.post.py_type}", file=sys.stderr)
            if post.post.py_type == "app.bsky.feed.defs#reasonRepost":
                print(f"Post is a repost", file=sys.stderr)

            messages = copy.deepcopy(characters["riot"].init_messages)

            messages.append({"role": "assistant", "content": f"^^Memory Subsystem: This is information about who I am: {characters['riot'].knowledge}^^"})
            messages.append({"role": "assistant", "content": f"^^Memory Subsystem: This is information about {post.post.author.handle}: {author.knowledge}^^"})
            
            question = "This post is unrelated to you, berch, or our stream. The author likely doesn't know us. Compose a nice or funny reply to the post. Don't ask them questions. Reply with only your exact comment."
            if len(images) > 0:
                messages.append({"role": "user", "content": f"^^Bluesky Subsystem: This is a post by {post.post.author.handle} with the attached images: {post.post.record.text}^^ {question} focus on the image if it's showing artwork.", "images": images})
            else:
                messages.append({"role": "user", "content": f"^^Bluesky Subsystem: This is a post by {post.post.author.handle}: {post.post.record.text}^^ {question}"})

            response = ollama_client.chat(model='Riot', messages=messages)
            new_tweet = re.sub(r'"', '', response['message']['content'])
            messages.append(response['message'])
            messages.append({"role": "user", "content": f"Are you sure you want to aopost this reply? If you think the post is nice, funny, or interesting, and not something that might be considered rude or self-promotional, reply with 'yes' If not, reply with 'no'"})
            response = ollama_client.chat(model='Riot', messages=messages)
            
            c.execute("INSERT INTO atprotoseen (id) VALUES (?) ON CONFLICT(id) DO NOTHING", (post.post.cid,))
            db.commit()
            wants_to_post = response['message']['content'].strip().lower() != "yes"
            print(f"wants to post: {wants_to_post}: {new_tweet}", file=sys.stderr)
...
def get_midi_mapping():
    midi_db = sqlite3.connect(resources + "stream.db", check_same_thread=False)
    c = midi_db.cursor()
...
def add_youtube_song():
    ...
    url = request.json.get('url').split('&')[0]
    dmca_risk = request.json.get('dmca_risk', 0)
    db = sqlite3.connect(resources + "stream.db")
    c = db.cursor()
    try:
        ydl_opts = {
