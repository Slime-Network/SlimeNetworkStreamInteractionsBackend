...
print(f"Resources path: {resources}")
...
class Character:
    def __init__(self, character):
        if isinstance(character, dict):
            ...
            self.atproto = character.get('atproto', '')
            ...
            self.atproto = character[7] if len(character) > 7 else ''
        self.doneTalking = 0
...
def init_db():
    print(os.path.abspath(resources + "riots-memory.db"))
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS atprotoseen
              (id text PRIMARY KEY)''')
...
def init_stream_db():
    db_stream = sqlite3.connect(resources + "stream.db", check_same_thread=False)
...
def get_characters_from_db():
    print("Loading characters from database...", resources + "riots-memory.db")
    db = sqlite3.connect(resources + "riots-memory.db", check_same_thread=False)
    c = db.cursor()
